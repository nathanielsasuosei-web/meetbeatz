...
export async function listActiveServices() {
  try {
    return await db.select().from(services).where(eq(services.isActive, true)).orderBy(asc(services.sortOrder), asc(services.id));
  } catch {
    return [];
  }
}
...
export async function listLicenseTypes(activeOnly = true) {
  try {
    ...
    if (activeOnly) return await query.where(eq(licenseTypes.isActive, true));
    return await query;
  } catch {
    return [];
  }
